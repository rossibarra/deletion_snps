...
for bed in range(0, len(mydelfiles)):
    ...
    mynamdelID = mydelfiles[bed].split('/')[-1].split('.')[0]
    mydelrangesdict[mynamdelID] = dict()
    ...
    for bedline in open(mybedFN):
        ...
        if mybedchr == myoldchr:
            if mybedstart <= myoldbedstop:
                ...
                #quit(1)
        if mybedchr not in mydelrangesdict[mynamdelID].keys():
            mydelrangesdict[mynamdelID][mybedchr] = list()
        mydelrangesdict[mynamdelID][mybedchr].append([int(mybedstart), int(mybedstop), 0])
    ...
    if myline[0] == '#':
        ...
        if ((len(myalts) == 2) and (myalts [0] == '*' or myalts[1] == '*')) or (len(myalts) ==1):
            ...
            if not referrorflag:
                ...
                # Initial test for heterozygous call
                for i in range(10, len(myline)):
                    ...
                    if myalleles[j] != myalleles[j+1]:
                            if myalleles[j] == '.' or myalleles[j+1] == '.':
                                ...
                                if (myalleles[j] == str(delpos)) or (myalleles[j+1] == str(delpos)):
                                    j = 0
                                else:
                ...
                for i in range(10, len(myline)):
                    ...
                    if mynamline in mydelrangesdict.keys():
                        if chrom in mydelrangesdict[mynamline].keys():
                            ...
                            awdelrange = []
                            awdelindex = -1
                            print('Chromosome ' + chrom + ' not found in ' + mynamline)
                        ...
                        if myalleles[j] == myalleles[j+1]:
                            ...
                            elif awdelflag and not namlinedelstatus:
                                if myalleles[j] in {'0', '1', '2'}:
                                    ambiguouscall = True
                                    myalleles[j] = myalleles[j+1] = awambiguouscall
                                    if not heterozygouscall:
                                        mydelrangesdict[mynamline][chrom][awdelindex][2] += 1
                            # Report weird sitatuations. Unexpected cases
                            elif not awdelflag and not namlinedelstatus:
                                # This captures pure homozygous calls
                                # For now do nothing
                                print('Homozygous call at: ' + mynamline + '\t' + chrom + '\t' + pos + '\t' + myalleles[0] + '/' + myalleles[1])
                                #j = 0
                            else:
                                print('Woah! homozygous call at: ' + mynamline + '\t' + chrom + '\t' + pos + '\t' + myalleles[0] + '/' + myalleles[1] + ' delstatus: ' + str(delstatus))
                        # Now check heterozygous SNP Calls and set Heterozygous calls
                            ...
                            # Print message if unexpected case for het call
                                ...
                                print('Weird heterozygous case at: ' + mynamline + '\t' + chrom + '\t' + pos + '\t' + myalleles[0] + '/' + myalleles[1])
                        namlines.append(myalleles)
                        #metadata = metadata + myline[i] + '\t'
                    else:
